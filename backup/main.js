...
function getOffsetMs(offsetStr) {
  const match = offsetStr.match(/([+-])(\d{2}):(\d{2})/);
  if (!match) return 0;
  const sign = match[1] === "-" ? -1 : 1;
  const hour = parseInt(match[2], 10);
  const min = parseInt(match[3], 10);
  return sign * (hour * 60 + min) * 60 * 1000;
}

function updateClocks() {
  const now = new Date();
  const leftTime = new Date(now.getTime() + getOffsetMs(offsetLeft) - now.getTimezoneOffset() * 60000);
  const rightTime = new Date(now.getTime() + getOffsetMs(offsetRight) - now.getTimezoneOffset() * 60000);
  updateClock(leftTime, "korea", "LEFT", hourFormat);
  updateClock(rightTime, "tanzania", "RIGHT", hourFormat);
}
